...
export type FixtureDetailPrediction = {
  user_id: string;
  username: string;
  avatar_id: string;
...
export async function getFixtureDetail(
  fixtureId: string,
  poolId: string,
): Promise<FixtureDetailPrediction[]> {
  const supabase = createServiceClient();
  ...
  // 2. Predicciones para este fixture
  const { data: preds } = await supabase
    .from("predictions")
    .select("user_id, pred_home, pred_away, points_won")
    .eq("fixture_id", fixtureId)
    .eq("pool_id", poolId);

  const predMap = Object.fromEntries((preds ?? []).map((p) => [p.user_id, p]));

  // 3. Combinar: todos los miembros, tengan predicción o no
  return members.map((m: any) => ({
    user_id: m.user_id,
    username: m.profile.username,
    avatar_id: m.profile.avatar_id,
    pred_home: predMap[m.user_id]?.pred_home ?? null,
